...
const PHOTO_PRODUCTS = new Set(["1", "2", "7", "8", "9", "10"])
...
const cardThemes: Record<string, { bg: string; text: string; accent: string; sub: string; shape: string }> = {
  ...
  "5": { bg: "#2D1052", text: "#EDE0FF", accent: "#F5C200", sub: "rgba(237,224,255,0.55)", shape: "rgba(245,194,0,0.1)" },
  "6": { bg: "#1B5E35", text: "#F7F2E4", accent: "#F5C200", sub: "rgba(247,242,228,0.55)", shape: "rgba(245,194,0,0.08)" },
    ...
    return (
      ...
      >
        ...
        <div className="absolute top-4 left-4 z-10">
          <span
            className="font-display font-bold text-xs tracking-widest px-3 py-1"
            style={{ backgroundColor: "#f5c200", color: "#1c2b12", borderRadius: "2px" }}
          >
            {categoryLabel}
          </span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 md:p-6 z-10">
          <h3
            ...
            style={{
              fontSize: heroSize ? "clamp(1.5rem, 3vw, 2.2rem)" : "1.2rem",
              color: "#ffffff",
          ...
          {ingNames.length > 0 && (
            <p className="font-display text-xs mb-3 tracking-wide" style={{ color: "rgba(255,255,255,0.6)" }}>
              {ingNames.join(" · ")}
            </p>
          )}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full p-5 md:p-6">
        <div className="flex items-start justify-between mb-auto">
          <span
            className="font-display font-bold text-xs tracking-widest px-2 py-1"
            style={{ backgroundColor: theme.accent, color: "#1c2b12", borderRadius: "2px", opacity: 0.95 }}
          >
            {categoryLabel}
          </span>
        ...
        <h3
          className="font-display font-bold leading-[0.9] mb-3 group-hover:opacity-90 transition-opacity"
          style={{ fontSize: heroSize ? "clamp(2rem, 4vw, 3.2rem)" : "clamp(1.4rem, 2.5vw, 2rem)", color: theme.text, letterSpacing: "-0.02em" }}
        >
          {product.name}
        </h3>

        {ingNames.length > 0 && (
          <p className="font-display text-xs mb-5 tracking-wide" style={{ color: theme.sub }}>
            {ingNames.join(" · ")}
          </p>
        )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col leading-tight">
            <span
              ...
              style={{ fontSize: heroSize ? "2rem" : "1.5rem", color: theme.accent === theme.text ? theme.text : theme.accent }}
            >
              ${product.price.toFixed(2)}
            </span>
            <span
              className="font-display font-semibold"
              style={{ fontSize: heroSize ? "0.9rem" : "0.75rem", color: theme.sub }}
            >
              {toPEN(product.pricePEN)}
            </span>
          </div>
